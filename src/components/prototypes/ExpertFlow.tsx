...
export default function ExpertFlow() {
  ...
  const capturingIndicator = phase === "during-call" ? (
  ...
  const renderRecordContent = () => (
    <div className="p-3 space-y-2">
      <div className="flex items-center gap-1.5 rounded-md border border-zinc-200 px-2 py-1.5">
        <span className={`h-2 w-2 rounded-full ${phase === "during-call" ? "bg-green-500 animate-pulse" : "bg-zinc-300"}`} />
        <span className="text-[10px] text-zinc-500">{phase === "during-call" ? "Recording" : "Idle"}</span>
      </div>
      <div className="space-y-1.5">
        <div className="text-[9px] font-semibold uppercase tracking-wider text-zinc-400">Capture Settings</div>
  ...
  const shadowPanel = (
    <ShadowRightRail
      defaultTab="Chat"
      headerExtra={capturingIndicator}
      renderChat={renderChatContent}
      renderRecord={renderRecordContent}
    />
  );
